window.addEventListener('DOMContentLoaded', () => { 
  ...
  function prepInput(lines) {
    var num_vars = parseInt(lines[0]);
    var numConstraints = parseInt(lines[1]);
    var numeratorZ = lines[2].split(' ').map(el => new Fraction(el));
    var denumeratorZ = lines[3].split(' ').map(el => new Fraction(el));
    var constraints = [];
  
    for (var row_num = 0; row_num < numConstraints; row_num++) {
      var line = (lines[4 + row_num]).split(' ');
      var constraint = line.map(el => new Fraction(el));
      constraints.push(constraint);
    }
  
    // ------------------------------------
    // --- приведение к линейной задаче ---   
    // ------------------------------------
  
    // переменных теперь становится на одну больше 
    num_vars++;
    // ограничений тоже
    numConstraints++;
    var gradF = numeratorZ;
    
    // столбец св членов делаю < 0
    constraints.forEach(row => {
      row[row.length - 1] = row[row.length - 1].mul(-1);
    });
  
    // добавляю знам-ль в ограничения
    constraints.push(denumeratorZ);
  
    // пришпандориваю столбец (0,...,1)
    ...
    // добавляю столбец со знаками
    constraints.forEach(row => {
      row.push(0);
    });
  
    // в методичке экстремум только один
    var extrType = "max";
  ...
  function optimizeBasis(stage, simplexTable, artBasVectorsCounter, deltaMaxInd, basis, coeffs) {
    ...
    let simplexTablePretty = new Array;
    for (let i=0;i<simplexTable.length;i++){
      simplexTablePretty[i] = new Array;
      for (let j=0;j<simplexTable[i].length;j++) {
        simplexTablePretty[i][j] = (simplexTable[i][j]['s']*simplexTable[i][j]['n'])+"/"+simplexTable[i][j]['d'];
      }
  }
  console.table(simplexTablePretty);
  ...
  document.getElementById("solve").addEventListener('click', function () {
    var lines = [];
    lines[0] = parseInt(document.getElementById("numVars").value);
    if (lines[0]==NaN) {
      document.getElementById("result").innerHTML = "Некорректное заполнение числа переменных";
      return;
    }
    lines[1] = parseInt(document.getElementById("numConstraints").value);
    if (lines[1]==NaN) {
    ...
    for (let i=0;i<=lines[0];i++){
      if (gradFEl[i].value=='') gradF.push(0);
      else
      if (isNaN(parseInt(gradFEl[i].value))) {
        document.getElementById("result").innerHTML = "Некорректное заполнение целевой функции";
        return;
      }
      else
      gradF.push(parseInt(gradFEl[i].value)); 
    }
    lines[2] = gradF.join(' ');
    gradF = [];
    for (let i=0;i<=lines[0];i++){
      if (gradFEl[i+lines[0]+1].value=='') gradF.push(0);
      else
      if (isNaN(parseInt(gradFEl[i+lines[0]+1].value))) {
        document.getElementById("result").innerHTML = "Некорректное заполнение целевой функции";
        return;
      }
      else
      gradF.push(parseInt(gradFEl[i+lines[0]+1].value)); 
    }
    lines[3] = gradF.join(' ');
    let constraintsEl = document.getElementsByClassName('constraintVarInp');
    let constraintsLastEl = document.getElementsByClassName('constraintLastInp');
    for (let i=0;i<lines[1];i++) {
      lines[4+i] = "";
      for (let j=0;j<lines[0];j++){
        if (constraintsEl[i*lines[0]+j].value=='') lines[4+i]+="0 ";
        else
        if (isNaN(parseInt(constraintsEl[i*lines[0]+j].value))) {
          document.getElementById("result").innerHTML = "Некорректное заполнение ограничений";
          return;
        }
        else
        lines[4+i]+=parseInt(constraintsEl[i*lines[0]+j].value)+" ";
      }
      if (constraintsLastEl[i].value=='') lines[4+i]+="0";
      else
      if (isNaN(parseInt(constraintsLastEl[i].value))) {
        document.getElementById("result").innerHTML = "Некорректное заполнение ограничений";
        return;
      }
      else
      lines[4+i]+=parseInt(constraintsLastEl[i].value);
    }
    //console.log(lines);
    var [lines, num_vars, numConstraints, gradFF, extrType, constraints] = prepInput(lines);
    document.getElementById("result").innerHTML = solve(num_vars, numConstraints, gradFF, extrType, constraints);
    return;
  });
})